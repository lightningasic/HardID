...
int main(void){
    ...
    uint8_t spk[25] = {0x76,0xa9,0x14,
        ...
        0x88,0xac};
    if (!script_to_addr(spk, 25, out, sizeof out)) { printf("FAIL p2pkh not decoded\n"); return 1; }
     ...
     * -> known address 3QJmV3qfvL9fmYo31kfejGEdAL6b2rQ3Ld  (well-known) */
    uint8_t sh[23] = {0xa9,0x14,
        ...
        0x87};
    if (!script_to_addr(sh, 23, out, sizeof out)) { printf("FAIL p2sh not decoded\n"); return 1; }
    printf("PASS P2SH -> %s\n", out);

    /* invalid length / wrong opcode still falls back to hex (addr_valid false) */
    uint8_t bad[25] = {0};
    bad[0] = 0x77; /* wrong first byte */
    if (script_to_addr(bad, 25, out, sizeof out)) { printf("FAIL invalid accepted\n"); return 1; }
