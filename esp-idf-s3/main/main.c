...
{
	/* Boot into FIDO serving when the FIDO app is active (default, so the
	 * device is detected by a browser the moment it is plugged in, like a
	 * YubiKey). If the user deleted the FIDO app, boot straight into the
	 * wallet menu instead. The FIDO screen has a BACK button that returns
